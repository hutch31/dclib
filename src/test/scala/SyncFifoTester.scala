...
class SyncFifoTestbench extends Module {
  ...
  val dut = Module(new SyncFifo(new ColorToken(1,16), 8))

  dut.io.deq_clock := clock
  dut.io.deq_reset := reset
...
class SyncFifoTester(tb: SyncFifoTestbench) extends PeekPokeTester(tb) {
