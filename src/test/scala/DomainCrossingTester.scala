...
class DCDomainCrossingTestbench extends Module {
  ...
  val dut = Module(new DCDomainCrossing(new ColorToken(1, 16)))
  //val dut = Module(new DCOutput(new ColorToken(1,16)))

  dut.io.deq_clock := clock
  dut.io.deq_reset := reset.toBool()
...
class DomainCrossingTester(tb: DCDomainCrossingTestbench) extends PeekPokeTester(tb) {
